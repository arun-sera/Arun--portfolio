...
You should see:
```
Portfolio server running at http://localhost:3007
```

**Step 6 — Open the site.**
- Public page: http://localhost:3007
...
- **Port already in use** → change `PORT=3007` in `.env` to something else, e.g. `3001`.
...
- **Public page shows "Couldn't load projects"** → the server isn't running, or you opened `index.html` directly as a file instead of via `http://localhost:3007`. Always access the site through the server URL, not by double-clicking the HTML file.
